...
class MArsipInaktif extends Model
{
    ...
    {
        return $this->db->table('ev_item_inaktif')->where('jenis_arsip', 'vital')
            ->countAllResults();
    }
    ...
    {
        return $this->db->table('ev_item_inaktif')
            ->join('km_klasifikasi', 'ev_item_inaktif.id_klasifikasi=km_klasifikasi.id')
            ->get()->getResultArray();
    }
    public function getIdData($id)
    {
        return $this->db->table('ev_item_inaktif')
            ->join('km_klasifikasi', 'ev_item_inaktif.id_klasifikasi=km_klasifikasi.id')
            ->join('km_sub_bidang', 'ev_item_inaktif.id_sub_bidang=km_sub_bidang.id')
            ->where('ev_item_inaktif.id', $id)
            ->get()->getRowArray();
    }
    public function getIdUser($id_user)
    {
        return $this->db->table('ev_item_inaktif')
            ->join('km_klasifikasi', 'ev_item_inaktif.id_klasifikasi=km_klasifikasi.id')
            ->join('km_sub_bidang', 'ev_item_inaktif.id_sub_bidang=km_sub_bidang.id')
            ->where('ev_item_inaktif.id_user', $id_user)
            ->get()->getResultArray();
    }
    // untuk peminjmaman 
    ...
    {
        return $this->db->table('ev_item_inaktif')
            ->select('ev_item_inaktif.id as id, nama_sub_bidang, judul_dokumen, no_dokumen, tgl_dokumen, jenis_arsip')
            ->join('km_klasifikasi', 'ev_item_inaktif.id_klasifikasi=km_klasifikasi.id', 'left')
            ->join('km_sub_bidang', 'ev_item_inaktif.id_sub_bidang=km_sub_bidang.id', 'left')
            ->where('ev_item_inaktif.id_user', $id_user)
            ->get()->getResultArray();
    }
    public function rincianJmlArsip()
    {
        $sql = "SELECT km_es2.nama_es2 as nama_unit, COUNT(ev_item_inaktif.id) as jml_arsip, km_es2.id as id_unit FROM ev_item_inaktif INNER JOIN users ON ev_item_inaktif.id_user=users.id INNER JOIN dd_hak_unit ON users.id=dd_hak_unit.id_user INNER JOIN km_es2 ON dd_hak_unit.parameter_unit=km_es2.id group by km_es2.id order by jml_arsip DESC;";
    ...
    public function arsipVital($id)
    {
        return $this->db->table('ev_item_inaktif')
            ...
            ->where('ev_item_inaktif.id_user', $id)
            ->get()->getResultArray();
    }
    public function getRekapitulasiArsipI()
    {
        $builder = $this->db->table('km_es2');
        $builder->select('km_es2.id AS es2_id, km_es2.nama_es2 as nama_unit, COUNT(ev_item_inaktif.id) AS jumlah_arsip')
            ->join('km_bidang', 'km_es2.id = km_bidang.id_es2')
            ->join('km_sub_bidang', 'km_bidang.id = km_sub_bidang.id_bidang')
            ->join('ev_item_inaktif', 'km_sub_bidang.id = ev_item_inaktif.id_sub_bidang', 'left')
    ...
    {
        return $this->db->table('ev_item_inaktif')
            ->select('ev_item_inaktif.id_user as id_user, kode_klasifikasi, no_berkas, nama_berkas, no_dokumen, judul_dokumen, tgl_dokumen, ev_item_inaktif.media_simpan as media, derajat_keamanan, jenis_arsip, tk_perkembangan, jumlah, ev_item_inaktif.lokasi_simpan, ev_item_inaktif.no_box, dasar_catat')
            ->join('km_klasifikasi', 'ev_item_inaktif.id_klasifikasi=km_klasifikasi.id', 'left')
            ->join('ev_berkas_aktif', 'ev_item_inaktif.id_berkas=ev_berkas_aktif.id', 'left')
            ->where('ev_item_inaktif.id_user', $id)
    ...
    public function getArsipInaktif($id)
    {
        ...
        $builder->select('ev_item_inaktif.*')
            ->join('km_klasifikasi', 'ev_item_inaktif.id_klasifikasi = km_klasifikasi.id')
            ->where('id_user', $id)
            ->where('YEAR(CURDATE()) - ev_item_inaktif.tahun_cipta > km_klasifikasi.umur_inaktif');
    ...
    {
        $builder = $this->db->table($this->table);
        $builder->select('ev_item_inaktif.*,umur_aktif, umur_inaktif')
            ->join('km_klasifikasi', 'ev_item_inaktif.id_klasifikasi = km_klasifikasi.id')
            ->where('id_user', $id)
            ->where('YEAR(CURDATE()) - ev_item_inaktif.tahun_cipta > km_klasifikasi.umur_inaktif');
